...
- **Phase 3 ✅** (2026-07-28) — scaffold complete: Vite 8 + React 19 + TS 7 + Tailwind 4.3; deps installed incl. `thinking-orbs`, `motion`, `@fontsource-variable/inter`; `useLocale` hook + placeholder App wired to content model; production build passes (64 KB JS gz). Assets moved to `public/`. `git init` done (no commits yet). Firebase project **`manuel-pacheco-portfolio`** created → will serve at `manuel-pacheco-portfolio.web.app`. GitHub Actions workflow written (needs GitHub repo + `FIREBASE_SERVICE_ACCOUNT` secret to activate).
